...
// Returns realistic base active-user count for the current hour (Thai usage pattern)
function getTimeBasedBase(): number {
  const h = new Date().getHours()
  // [hour] => approximate active users
  const curve: Record<number, number> = {
    0: 2400, 1: 1600, 2: 900, 3: 600, 4: 500, 5: 700,
    6: 1200, 7: 1700, 8: 2200, 9: 2800, 10: 3200, 11: 3500,
    12: 3800, 13: 3600, 14: 3200, 15: 3000, 16: 3400, 17: 4200,
    18: 4800, 19: 5100, 20: 4600, 21: 4000, 22: 3200, 23: 2800,
  }
  return curve[h] ?? 3000
}

function useFakeLiveCount(variance: number, intervalMs: number) {
  const [count, setCount] = useState(getTimeBasedBase)
  useEffect(() => {
    const tick = () => {
      const base = getTimeBasedBase()
      setCount(base + Math.floor((Math.random() - 0.5) * variance * 2))
    }
...
function Form({ avatarUrls }: { avatarUrls: string[] }) {
  const liveCount = useFakeLiveCount(120, 4000)
  ...
  // Auto-clear stale auth cookies when redirected back with oauth error
  const oauthError = searchParams.get('error') === 'oauth'
  if (oauthError && typeof document !== 'undefined') {
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex -space-x-2.5">
                  {avatarUrls.map((url, i) => (
                    <div
                      key={i}
                      className={`w-8 h-8 rounded-full bg-gradient-to-br ${AVATAR_COLORS[i % AVATAR_COLORS.length]} p-[1.5px] ring-2 ring-black/60`}
                  ...
                  {/* +N bubble */}
                  <div
                    className="w-8 h-8 rounded-full bg-white/[0.07] border border-white/[0.12] flex items-center justify-center ring-2 ring-black/60"
                    style={{ zIndex: 0 }}
                  >
                    <span className="text-[10px] font-bold text-white/50">+99</span>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-0.5">
                  ...
                  <p className="text-[11px] text-white/30 leading-none">
                    <span className="text-white/60 font-semibold tabular-nums transition-all duration-700">{liveCount.toLocaleString()} คน</span> กำลังร่วมคาดการณ์อยู่ตอนนี้
                  </p>
                </div>
              </div>
            </div>
...
export default function LoginForm({ avatarUrls }: { avatarUrls: string[] }) {
  return (
    <Suspense>
      <Form avatarUrls={avatarUrls} />
    </Suspense>
  )
}
